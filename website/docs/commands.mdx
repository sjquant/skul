...
| Command | Description |
| --- | --- |
| `skul shadow --suspend` | Restore tracked `AGENTS.md` or `CLAUDE.md` files back to `HEAD` and clear shadow state from the working tree |
...
| `skul clear-cache [source]` | Remove one cached remote source |
| `skul clear-cache --all` | Remove all cached remote sources |

## Flags

- Mutating commands accept `--dry-run`.
- `skul list`, `skul status`, and `skul check` accept `--json`.
- `skul add` accepts `--ssh`.
- Repeat `--agent` to target multiple tools.
- Repeat `--include <item>` to install selected bundle items only.
- Use `--select-items` to choose bundle items interactively.

Bundle item selectors are `skills/<name>`, `commands/<name>`, `agents/<name>`, and `root-instruction`. `AGENTS.md` and `CLAUDE.md` are also accepted as root-instruction aliases. If `--include` and `--select-items` are used together, the included items are preselected in the picker.
