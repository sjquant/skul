...
- [TODO] Implement bundle structure auto-detection and cross-tool content transforms at materialization time.

  **Bundle structure convention:**
  - A bundle directory may contain generic subdirectories (`skills/`, `commands/`, `agents/`) — these are authored in Claude Code canonical format (no dotdir prefix).
  - A bundle directory may also contain tool-native dotdirs (`.cursor/`, `.opencode/`, `.agents/`, `.codex/`, `.claude/`) — these are pre-authored in that tool's native format and are copied verbatim.
  - Both forms may coexist in the same bundle.

  **Manifest auto-generation:**
  - `manifest.json` is NOT written by bundle authors. It is generated by skul when a bundle is fetched or registered, by scanning the bundle directory and matching paths against tool definitions from `tool-mapping.ts`.
  - Implement `inferBundleManifest(bundleDir)` that scans for canonical dirs and dotdirs, produces a `BundleManifest["tools"]` map automatically, and writes `manifest.json` to the bundle cache directory.

  **Materialization transform rule:**
  - When materializing for tool X, for each source path in the bundle:
    - If the source path starts with tool X's native dotdir (e.g. `.cursor/` for cursor) → raw copy to user's project, preserving relative path under the dotdir.
    - If the source path is a canonical dir (`skills/`, `commands/`, `agents/`) → apply cross-tool transform using the existing functions in `bundle-translation.ts` (`translateSkill`, `translateCommand`, `translateAgent`) and write to tool X's native directory in the user's project.
  - Output always lands in the tool's native directory (`.cursor/skills/`, `.agents/skills/`, `.codex/agents/`, `.opencode/skills/`, etc.) regardless of source form.

  **Wiring:**
  - Wire `translateSkill`, `translateCommand`, `translateAgent` from `bundle-translation.ts` into `materializeBundle` in `bundle-materialization.ts`.
  - Add tests covering: canonical-to-cursor, canonical-to-codex (TOML agents, openai.yaml policy files), canonical-to-opencode (compatibility metadata, manualOnly → command), native dotdir passthrough for each tool.
  - Document the transform matrix in a stable location under `docs/`.
